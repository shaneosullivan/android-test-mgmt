...
export async function downloadImageAsBase64(imageUrl: string): Promise<string | null> {
  try {
    ...
    if (metadata.width && metadata.height && (metadata.width > 100 || metadata.height > 100)) {
      console.log(`Resizing image from ${metadata.width}x${metadata.height} to fit within 100x100`);
    ...
    console.log(`Successfully processed image to base64. Final size: ${Math.round(processedBuffer.length / 1024)}KB`);
    return dataUrl;
